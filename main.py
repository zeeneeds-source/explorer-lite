...
app = FastAPI(title="Kogenta Explorer Lite (Demo)")
...
@app.get("/api/poi")
def poi_search(q: str = Query("", description="keyword to search e.g. 'ASDA'")):
    ql = q.lower().strip()
...
def busyness(lat: float, lon: float):
    ...
    # generate a realistic-ish 0-100 score
    base = 50 + 40*math.sin((lat+lon)*3.14) + rnd.randint(-10, 10)
    score = max(0, min(100, int(base)))
    return {"score": score, "ts": datetime.utcnow().isoformat() + "Z"}
...
def flows(lat: float, lon: float):
    ...
    data.sort(key=lambda x: x["visitors"], reverse=True)
    return {"results": data}
...
@app.get("/")
def root():
    return {"status": "ok", "see": "/web/ or /docs"}